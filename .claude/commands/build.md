...
2. Save a checkpoint FIRST (`python tools/ue.py snapshot`), then take a "before"
   screenshot (`python tools/ue.py screenshot`). The checkpoint is my safety net:
   /undo can cleanly remove everything you're about to add.
...
4. After each visible milestone: screenshot, read the image, and course-correct.
   ...
   must line up.
5. Finish with a 3-angle QA pass (`python tools/ue.py sweep`) — check the top-down
   for layout problems, eye-level for looks, player-eye for feel — fix what's off,
   and tell me in 2-3 plain sentences what you built and what I might want next.
